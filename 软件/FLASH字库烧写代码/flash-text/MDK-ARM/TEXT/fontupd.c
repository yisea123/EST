...
//字库存放起始地址 
#define FONTINFOADDR 	1024*1024*28		//字库存放首地址
//定义各个字库的大小
...
u8 updata_fontx(u16 x,u16 y,u8 size,u8 fx)
{
  ...
  u32 fsize=0;
		...
		fupd_prog(x+16*size/2,y,size,fsize,offx);	 			//进度显示
		while(1)//死循环执行
		{		
	 		if(GBK_OVER_Flag)
			{			
			...
			delay_u(100);
			if(GBK_OVER_Flag>(WATE_TIME+10)*50)   //超过正常时间10ms则说明此字库发送完毕
				break;
	 	}		
		...
		usart6_init(BAUD_RATE);		//重新初始化串口及DMA
		GBK_OVER_Flag=0;
...
{	
	...
	//	LCD_ShowString(x,y,240,320,size,(u8*)"Erasing sectors... ");//提示正在擦除扇区	
		LCD_ShowString(x,y,(u8*)"Erasing sectors... ",size,1);
	LCD_refresh();
		for(i=0;i<FONTSECSIZE;i++)	//先擦除字库区域,提高写入速度
		{
      fupd_prog(x+16*size/2,y,size,FONTSECSIZE,i);//进度显示
...
//			else
//			{
//				LCD_ShowString(x,y+15,(u8*)"no ",size,1);
//				LCD_refresh();
//			}
		}
//		LCD_ShowString(x,y+30,(u8*)"Updating UNIGBK.BIN  ",size,1);
//	LCD_refresh();
//		delay_ms(100);
		
	//	LCD_ShowString(x,y,240,320,size,(u8*)"Updating UNIGBK.BIN  ");
		LCD_ShowString(x,y,(u8*)"Updating UNIGBK.BIN  ",size,1);
	LCD_refresh();
		updata_fontx(x,y+15,size,0);	//更新GBK12.FON
	//	LCD_ShowString(x,y,240,320,size,(u8*)"Updating GBK12.FON  ");
				LCD_ShowString(x,y,(u8*)"Updating GBK12.FON  ",size,1);
		LCD_refresh();
		updata_fontx(x,y+15,size,1);	//更新GBK12.FON
	//	LCD_ShowString(x,y,240,320,size,(u8*)"Updating GBK16.FON  ");
		LCD_ShowString(x,y,(u8*)"Updating GBK16.FON  ",size,1);
		LCD_refresh();
		updata_fontx(x,y+15,size,2);	//更新GBK16.FON
	//	LCD_ShowString(x,y,240,320,size,(u8*)"Updating GBK24.FON  ");
		LCD_ShowString(x,y,(u8*)"Updating GBK24.FON  ",size,1);
		LCD_refresh();
		updata_fontx(x,y+15,size,3);	//更新GBK16.FON
		//全部更新好了
		ftinfo.fontok=0XAA;
	...
	//	LCD_Clear(WHITE);
//		LCD_clear();
////		LCD_ShowString(x,y+40,(u8*)"对应汉字(12*12)为:一往无前",12,1);
////		LCD_ShowString(x,y+60,(u8*)"对应汉字(12*12)为:一往无前",16,1);
////		LCD_ShowString(x,y+100,(u8*)"对应汉字(12*12)为:一往无前",24,1);
//		Show_Str(x,y,150,12,"对应汉字12为:一往无前",12,1);	
//	Show_Str(x,y+20,180,16,"对应汉字16为:一往无前",16,1);			 
//	Show_Str(x,y+50,180,24,"对应汉字为24:一往无前",24,1);
//	LCD_refresh();	
		return 0;
}
