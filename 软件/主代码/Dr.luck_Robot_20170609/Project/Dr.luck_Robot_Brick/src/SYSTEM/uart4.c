...
UBYTE Uart1RecBuf[UART_RECBUF_SIZE];     //接收缓冲
u16 Uart1RecBufIn = 0 ;
...
void uart4_init(u32 bound){
  ...
  GPIO_InitTypeDef GPIO_InitStructure;
	...
	RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOA | RCC_AHB1Periph_GPIOF,ENABLE); //使能GPIOB时钟
  ...
  GPIO_Init(GPIOF, &GPIO_InitStructure);										//初始化
	...
	GPIO_ResetBits(GPIOF,GPIO_Pin_10);  //拉低使能
...
void UART4_IRQHandler(void)                	//串口1中断服务程序
{
	...
	if(USART_GetITStatus(UART4, USART_IT_RXNE) != RESET)  //接收中断
	{
		...
		Uart1RecBuf[Uart1RecBufIn] = USART_ReceiveData(UART4);			//接收字符存入数组
