/**
  ******************************************************************************
  * @file    EV3 Brick of Dr.luck/vs10xx.c 
...
{
	SPI3_SetSpeed(SPI_BaudRatePrescaler_32);//设置到低速模式 
}
//SD卡正常工作的时候,可以高速了
...
void VS_Init(void)
{
	GPIO_InitTypeDef  GPIO_InitStructure;
 	RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOG|RCC_AHB1Periph_GPIOA, ENABLE);	 //使能PC,PE,PF端口时钟
	...
	GPIO_Init(GPIOG, &GPIO_InitStructure);
 	...
 	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_4|GPIO_Pin_8;	//PA4,PA8
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT; 		 //推挽输出
	GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;
...
{
    u16 volt=0; 			//暂存音量值
    volt=100-volx;			//取反一下,得到最大值,表示最大的表示 
	volt<<=8;
    volt+=100-volx;			//得到音量设置后大小
...
void Mp3Reset(void)
{
	...
	delay(10);
	wr_command(0x0b,sound_volume,sound_volume);   // 音量
	delay(10);
	spi_write(0);
	spi_write(0);
	spi_write(0);
...
{
	while(DREQ() == 0);  //VS1003的DREQ为高才能写入数据
	spi_write(dat);//通过SPI向VS1003写入一个字节的音频数据
}
//void flush_buffer(void) 
//{	unsigned int i;
//	XDCS_0();	   //打开数据片选，即开启SDI传输
//	for(i=0;i<2048;i++)
//	{
//		send_dat(0);
//	}
...
u8 flush_buffer(void)
{
	unsigned int i;
	static u16 clean_index=0;
