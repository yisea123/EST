...
uint32_t USBD_OTG_ISR_Handler (USB_OTG_CORE_HANDLE *pdev)
{
  ...
  if (USB_OTG_IsDeviceMode(pdev)) /* ensure that we are in device mode */
  {
    ...
    if (gintr_status.b.outepintr)	//接收中断
    {
      retval |= DCD_HandleOutEP_ISR(pdev);//主到从函数
    }    
    
    if (gintr_status.b.inepint)	//发送完成中断
    {
      retval |= DCD_HandleInEP_ISR(pdev);//从到主函数
    }
    ...
    if (gintr_status.b.rxstsqlvl)	//接收中断
    {
