...
void USBH_Process(USB_OTG_CORE_HANDLE *pdev , USBH_HOST *phost)
{
  ...
  case HOST_IDLE :		//空闲状态
    ...
    if (HCD_IsDeviceConnected(pdev))  //如果 设备连接上
    {
      phost->gState = HOST_DEV_ATTACHED;	//把状态转入HOST_DEV_ATTACHED
      USB_OTG_BSP_mDelay(600);
    }
    break;
   
  case HOST_DEV_ATTACHED :		//接触状态（但没有连接上）
  ...
  case HOST_CLASS:   //主机类
    ...
    status = phost->class_cb->Machine(pdev, phost);		// USBH_HID_Handle
